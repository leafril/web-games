...
const ICE_FLOOR_FRAME_L = "L";
const ICE_FLOOR_FRAME_R = "R";
...
export class GameScene extends Phaser.Scene {
  ...
  private iceFloorSrcWidth = 1; // 고드름 바 원본 폭(프레임 분할 시 채움)
  ...
  /** 고드름 바 텍스처를 좌/우 반 프레임으로 1회 분할 — 문짝 2개에 각각 매핑(스윙 유지). */
  private ensureIceFloorFrames() {
    const tex = this.textures.get(ICE_FLOOR_TEXTURE);
    const src = tex.getSourceImage() as { width: number; height: number };
    this.iceFloorSrcWidth = src.width;
    if (tex.has(ICE_FLOOR_FRAME_L)) {
      return;
    }
    const halfW = Math.floor(src.width / 2);
    tex.add(ICE_FLOOR_FRAME_L, 0, 0, 0, halfW, src.height);
    tex.add(ICE_FLOOR_FRAME_R, 0, halfW, 0, src.width - halfW, src.height);
  }
